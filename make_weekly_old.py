...
from lucky_items import get_daily_items  # 주간도 일간과 동일하게 사용
...
from make_daily import (
    ...
    build_page1, build_page2_mbti, build_page3_monthly_flow,
    build_page4_astro, build_page5_lucky,
)
...
class WeeklyNumberedCanvas(rl_canvas.Canvas):
    footer_template = '루밍의 MBTI 타로 운세  |  주간 운세  |  {name}님 전용  |  {n}/{total}'
    customer_name = '고객'
...
def build_weekly_page6(story, customer, profile, card_data, direction, card_key,
    ...
    rows = [
        ['운세', '오늘 이 운세가 말하는 것'],
        [f"타로 · {card_data.get('kr', card_data['name'])}", intg['rows']['tarot']],
        [f"MBTI · {mbti}",                                    intg['rows']['mbti']],
        [f"사주 · {profile.get('ELEMENT', '')} 일간",          intg['rows']['saju']],
        [f"별자리 · {profile.get('STAR', '')}",                intg['rows']['star']],
        [f"띠 · {profile.get('ZOD', '')}",                     intg['rows']['zod']],
    ]
    ...
    # ─── 작성자/사업자 정보 박스 (마지막 7일차에만) ───
    if day_num == total_days and author_info:
        story.append(sp(18))
...
def make_weekly_pdf(customer, start_date=None, output_path=None, author_info=None):
    ...
    for i in range(7):
        ...
        lucky = get_daily_items(profile, card_key, d)
        daily_data.append({
            'date': d,
    ...
    for day_idx, d in enumerate(daily_data):
        ...
        # p1~p5: 일간과 동일 (재활용)
        build_page1(story, customer, profile,
        ...
        build_page4_astro(story, customer, profile,
                          d['card_key'], d['date'], styles)
        build_page5_lucky(story, customer, profile, d['lucky'],
                          d['card_key'], styles)
    ...
    doc.build(story, canvasmaker=WeeklyNumberedCanvas)
    return output_path
